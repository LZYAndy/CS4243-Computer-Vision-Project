...
import pickle
from haar import evaluate_wenda
from sift_match import feature_matching
from svm import SVM


def detect_wenda(visual=False, img_loc='datasets/ImageSets/val.txt'):
	dir_name = 'templates/'
	bboxes = []
	threshold = 0.5
	open('results/eval/wenda.txt', 'w').close()
	svm = SVM('wenda')

	# Run Wenda detection on each target image
	...
	for x in val_im_num:
		...
		haar_results = evaluate_wenda(image_num) # returns a list of bbox
		...
		for x1,y1,x2,y2 in final_bbox:
			...
			with open('results/eval/wenda.txt', 'a') as txt:
				txt.write(image_num + ' 0.000 ' + str(x1) + ' ' + str(y1) + ' ' + str(x2) + ' ' + str(y2) + '\n')

		cv2.imwrite("results/wenda/result_" + image_num + '.jpg',img)
		if visual:
